...
public class EmpleadoServiceImpl implements EmpleadoService {
    ...
    public String registrarEmpleado(EmpleadoRegistroDTO dto) {
        ...
        for (String roleName : dto.getRoles()) {
            if (isAdmin && !isDeveloper && (roleName.equals("ROLE_ADMIN") || roleName.equals("ROLE_DEVELOPER"))) {
                throw new org.springframework.security.access.AccessDeniedException("Un administrador no puede crear perfiles con el rol " + roleName);
            }
            
            Role role = roleRepository.findByRoleName(roleName)
                    .orElseThrow(() -> new IllegalArgumentException("El rol " + roleName + " no existe en el sistema."));
    ...
    public java.util.List<java.util.Map<String, Object>> obtenerTecnicos() {
        java.util.List<Empleado> tecnicos = empleadoRepository.findAllByRoleNameNative("ROLE_TECNICO");
    ...
    @Override
    @Transactional(readOnly = true)
    public org.springframework.data.domain.Page<pe.idat.BackEndConecta.dto.EmpleadoListDTO> obtenerEmpleadosPaginados(
        ...
        return empleadoRepository.buscarEmpleadosEstricto(rol, tipoDocumento, criterio, valor, pageable)
                .map(emp -> pe.idat.BackEndConecta.dto.EmpleadoListDTO.builder()
                        .id(emp.getId())
                        .documento(emp.getDocumento())
                        .tipoDocumento(emp.getTipoDocumento())
                        .nombresCompletos(emp.getNombres() + " " + emp.getApellidoPaterno() + " " + (emp.getApellidoMaterno() != null ? emp.getApellidoMaterno() : ""))
                        .username(emp.getUsername())
                        .celular(emp.getCelular())
                        .correo(emp.getCorreo())
                        .roles(emp.getRoles().stream().map(Role::getRoleName).collect(java.util.stream.Collectors.toSet()))
                        .build());
    }
}
